...
def plot_map(p, lons, lats, t0, alt=0, lat_0=69, lon_0=16):
    ...
    cs = m.pcolormesh(x, y, 1.0 - p, vmin=0, vmax=1.0, cmap="inferno")
    cl = plt.contour(x, y, p * 100, 7, colors='k')
    # Label levels with specially formatted floats
    if plt.rcParams["text.usetex"]:
        fmt = r'%.0f \%%'
    else:
        fmt = '%.0f %%'
    plt.clabel(cl, inline=1, fmt=fmt, fontsize=14)
    # add colorbar.
    cbar = m.colorbar(cs, location='bottom', pad="5%")
    plt.title("Fraction of sunlight at %1.2f km\n%s (UTC)" % (alt / 1e3, t0))
    fname = "eclipse-%f.jpg" % (float(t0))
    plt.text(1e5, 1e5, "Ahmed Ammar: www.sat.tn", color="b", weight='bold')
...
if __name__ == "__main__":
    for i in range(20):
        p = get_eclipse(ephem.date((2015, 3, 20, 7, 30, 0)) + ephem.second * 900 * i,
                        lats=n.linspace(0, 90, num=200), lons=n.linspace(-45, 45, num=200))

        plot_map(p, lats=n.linspace(0, 90, num=200), lons=n.linspace(-45, 45, num=200),
                 t0=ephem.date((2015, 3, 20, 7, 30, 0)) + ephem.second * 900 * i, alt=0, lat_0=45, lon_0=10)
